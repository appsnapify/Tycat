...
import { createClient } from '@/lib/supabase/server'; // ✅ Usar cliente servidor unificado
...
// ✅ NOVA: Função helper para chamadas RPC resilientes no servidor
const callServerRPCWithRetry = async (supabaseClient: any, funcName: string, params: any, retries = 2) => {
    for (let i = 0; i <= retries; i++) {
        try {
            const { data, error } = await supabaseClient.rpc(funcName, params);
            
            if (error) {
                // Se for erro de permissão e ainda temos tentativas, aguardar um pouco
                if (error.code === '42501' && i < retries) {
                    console.log(`[Server ${funcName}] Permission denied, retrying (attempt ${i + 1})`);
            ...
            if (i === retries) {
                console.error(`[Server ${funcName}] Failed after ${retries + 1} attempts:`, err);
                throw err;
            }
            console.log(`[Server ${funcName}] Attempt ${i + 1} failed, retrying...`);
...
async function fetchGuestStats(eventId: string, supabaseClient: any) {
  ...
  try {
    const { data, error } = await callServerRPCWithRetry(supabaseClient, 'get_guest_stats_for_event', { p_event_id: eventId });
    ...
    console.error("[Stats] Erro crítico ao buscar estatísticas de convidados:", error);
    return { totalGuests: 0, totalCheckedIn: 0 };
  }
}
...
async function fetchTopTeamsStats(eventId: string, supabaseClient: any) {
  ...
  try {
    const { data, error } = await callServerRPCWithRetry(supabaseClient, 'get_top_teams_for_event', { p_event_id: eventId });
    ...
    console.error("[Stats] Erro crítico ao buscar top equipas:", error);
    return [];
  }
}
...
async function fetchTopPromotersStats(eventId: string, supabaseClient: any) {
  ...
  try {
    const { data, error } = await callServerRPCWithRetry(supabaseClient, 'get_top_promoters_for_event', { p_event_id: eventId });
    ...
    console.error("[Stats] Erro crítico ao buscar top promotores:", error);
    return [];
  }
}
...
async function fetchGenderStats(eventId: string, supabaseClient: any) {
  ...
  try {
    const { data, error } = await callServerRPCWithRetry(supabaseClient, 'get_gender_stats_for_event', { p_event_id: eventId });
    ...
    console.error("[Stats] Erro crítico ao buscar estatísticas de gênero:", error);
    return { genderData: [] };
  }
}
...
export default async function EventoDetalhesPage({ params }: PageProps) {
  // ✅ Usar cliente servidor unificado
