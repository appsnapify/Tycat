 ...
 * @param filePath - Caminho do arquivo para validar
 * @param allowedDirectories - Diretórios permitidos
 * @returns True se o caminho for seguro
 */
function isSecurePath(filePath: string, allowedDirectories: string[]): boolean {
  try {
...
export async function runMigration(migrationName: string) {
  try {
    ...
    // Ler o conteúdo do arquivo de migração de forma segura
